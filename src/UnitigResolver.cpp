...
struct ResolveTriplet
{
	...
	ResolveTriplet(std::pair<size_t, bool> left, std::pair<size_t, bool> right, size_t coverage, bool forced) :
		left(left),
		right(right),
		coverage(coverage),
		forced(forced)
...
bool nodeIsForced(const ResolvableUnitigGraph& resolvableGraph, const std::vector<PathGroup>& readPaths, size_t node)
{
	for (std::pair<uint32_t, uint32_t> pair : resolvableGraph.iterateCrossingReads(node, readPaths))
	{
		size_t pathi = pair.first;
		for (const auto& read : readPaths[pathi].reads)
		{
			if (read.forced) return true;
		}
	}
	return false;
}

bool edgeIsForced(const ResolvableUnitigGraph& resolvableGraph, const std::vector<PathGroup>& readPaths, std::pair<size_t, bool> from, std::pair<size_t, bool> to)
{
	...
	bool lastHadForced = false;
	size_t lastChecked = std::numeric_limits<size_t>::max();
	for (const std::pair<uint32_t, uint32_t> pospair : resolvableGraph.iterateCrossingReads(from.first, readPaths))
	{
		const size_t i = pospair.first;
		const size_t j = pospair.second;
		if (i == lastChecked && !lastHadForced) return false;
		if (i != lastChecked)
		{
			bool hasForced = false;
			for (const auto& read : readPaths[i].reads)
			{
				if (read.forced) hasForced = true;
			}
			lastChecked = i;
			lastHadForced = hasForced;
...
ResolvableUnitigGraph getUnitigs(const UnitigGraph& initial, size_t minCoverage, const HashList& hashlist, const size_t kmerSize, const bool keepGaps, const std::vector<ReadPath>& rawPaths, std::ostream& log)
{
	...
	{
		if (!path.forced) continue;
		for (size_t i = 1; i < path.path.size(); i++)
		{
			std::pair<size_t, bool> from = path.path[i-1];
			std::pair<size_t, bool> to = path.path[i];
			auto canonpair = canon(from, to);
			result.overlaps[canonpair] = 0;
	...
	log << "estimated average coverage " << result.averageCoverage << std::endl;
	return result;
}
...
std::vector<ResolveTriplet> getRawTriplets(const ResolvableUnitigGraph& resolvableGraph, const phmap::flat_hash_set<size_t>& resolvables, const std::vector<PathGroup>& readPaths, size_t node, size_t minCoverage, bool partTriplets)
{
	...
	phmap::flat_hash_set<std::pair<std::pair<size_t, bool>, std::pair<size_t, bool>>> forcedTriplets;
	for (const std::pair<uint32_t, uint32_t> pospair : resolvableGraph.iterateCrossingReads(node, readPaths))
	{
		...
		if (j == 0 && resolvableGraph.edges[reverse(readPaths[i].path[0])].size() == 0)
		{
			...
			else
			{
				...
				for (const auto& read : readPaths[i].reads)
				{
					if (read.rightClip >= shouldCheckTrimAmount) continue;
					coverageHere += 1;
					if (read.forced) hasForced = true;
				}
			}
			if (coverageHere > 0)
			{
				...
				if (hasForced) forcedTriplets.insert(std::make_pair(left, right));
			}
		}
		if (j == readPaths[i].path.size()-1 && resolvableGraph.edges[readPaths[i].path.back()].size() == 0)
		{
			...
			else
			{
				...
				for (const auto& read : readPaths[i].reads)
				{
					if (read.leftClip >= shouldCheckTrimAmount) continue;
					coverageHere += 1;
					if (read.forced) hasForced = true;
				}
			}
		...
		if (j > 0 && j < readPaths[i].path.size()-1)
		{
			...
			else
			{
				...
				for (const auto& read : readPaths[i].reads)
				{
					if ((shouldCheckLeftTrimAmount == 0 || read.leftClip < shouldCheckLeftTrimAmount) && (shouldCheckRightTrimAmount == 0 || read.rightClip < shouldCheckRightTrimAmount))
					{
						coverageHere += 1;
						if (read.forced) hasForced = true;
					}
	...
	{
		if (pair.second < minCoverage && forcedTriplets.count(pair.first) == 0) continue;
		coveredTriplets.emplace_back(pair.first.first, pair.first.second, pair.second, forcedTriplets.count(pair.first));
	}
	if (partTriplets && resolvableGraph.edges[std::make_pair(node, true)].size() >= 1 && resolvableGraph.edges[std::make_pair(node, false)].size() >= 1)
	{
...
std::vector<ResolveTriplet> getGuessworkTriplets(const ResolvableUnitigGraph& resolvableGraph, const phmap::flat_hash_set<size_t>& resolvables, const std::vector<PathGroup>& readPaths, size_t node, size_t minCoverage, bool unconditional)
{
	...
	for (auto inpair : inneighborCopyCounts)
	{
		...
		for (auto outpair : outneighborCopyCounts)
		{
			...
			coveredTriplets.emplace_back(reverse(inpair.first), outpair.first, 0, false);
			addedGuesses += 1;
		}
	}
...
void filterCopyCountTriplets(const ResolvableUnitigGraph& resolvableGraph, const std::vector<PathGroup>& readPaths, const size_t node, std::vector<ResolveTriplet>& originals)
{
	...
	assert(threshold > 0);
	for (size_t i = originals.size()-1; i >= threshold; i--)
	{
		if (originals[i].forced) continue;
		std::swap(originals[i], originals.back());
		originals.pop_back();
	}
}
...
ResolutionResult resolve(ResolvableUnitigGraph& resolvableGraph, const HashList& hashlist, std::vector<PathGroup>& readPaths, const phmap::flat_hash_set<size_t>& resolvables, const size_t minCoverage, const bool unconditional, const bool guesswork, const bool copycountFilterHeuristic, std::ostream& log)
{
	...
	for (auto node : resolvables)
	{
		for (auto edge : resolvableGraph.edges[std::make_pair(node, true)])
		{
			...
			{
				log << "unresolvable even length exact palindrome frozen, id: " << node << std::endl;
...
UntippingResult tryRemoveCrosslinks(ResolvableUnitigGraph& resolvableGraph, std::vector<PathGroup>& readPaths, const double maxRemovableCoverage, const double minSafeCoverage, const std::pair<size_t, bool> start)
{
	...
	for (auto edge : resolvableGraph.edges[start])
	{
		...
		{
			if (!edgeIsForced(resolvableGraph, readPaths, start, edge))
			{
...
UntippingResult removeLowCoverageTips(ResolvableUnitigGraph& resolvableGraph, std::vector<PathGroup>& readPaths, const HashList& hashlist, const double maxRemovableCoverage, const double minSafeCoverage, const size_t maxRemovableLength, const phmap::flat_hash_set<size_t>& maybeUntippable)
{
	...
	for (size_t i = resolvableGraph.lastTippableChecked; i < resolvableGraph.unitigs.size(); i++)
	{
		...
		if (nodeIsForced(resolvableGraph, readPaths, i)) continue;
...
CleaningResult cleanComponent(ResolvableUnitigGraph& resolvableGraph, std::vector<PathGroup>& readPaths, const size_t minLongLength, const size_t minUnresolvableLength, const size_t maxUnresolvableLength, std::pair<size_t, bool> start)
{
	...
	for (auto pair : componentNodeSides)
	{
		for (auto edge : resolvableGraph.edges[pair])
		{
			int coverage = getEdgeCoverage(resolvableGraph, readPaths, pair, edge);
			size_t edgeCopyCount = (coverage + resolvableGraph.averageCoverage / 2) / resolvableGraph.averageCoverage;
			if (edgeCopyCount == 0 && !edgeIsForced(resolvableGraph, readPaths, pair, edge)) removedEdges.insert(canon(pair, edge));
		}
		double coverage = resolvableGraph.getCoverage(readPaths, pair.first);
		size_t estimatedCopyCount = (coverage + resolvableGraph.averageCoverage / 2) / resolvableGraph.averageCoverage;
		if (estimatedCopyCount == 0 && !nodeIsForced(resolvableGraph, readPaths, pair.first)) removedNodes.insert(pair.first);
	}
...
void maybeTrim(ResolvableUnitigGraph& resolvableGraph, std::vector<PathGroup>& readPaths, std::unordered_set<size_t>& maybeUnitigifiable, std::pair<size_t, bool> pos, size_t maxTrim, std::ostream& log)
{
	if (maxTrim == 0) return;
	...
	{
		log << "removed trim problem node " << pos.first << std::endl;
		std::vector<std::pair<std::pair<size_t, bool>, size_t>> alsoTrim;
		for (auto edge : resolvableGraph.edges[reverse(pos)])
		{
			maybeUnitigifiable.insert(edge.first);
		...
		for (auto pair : alsoTrim)
		{
			...
			maybeTrim(resolvableGraph, readPaths, maybeUnitigifiable, pair.first, pair.second, log);
		}
		return;
	}
...
std::unordered_set<size_t> trimNodes(std::vector<std::pair<std::pair<size_t, bool>, size_t>> maybeTrimmable, ResolvableUnitigGraph& resolvableGraph, std::vector<PathGroup>& readPaths, std::ostream& log)
{
...
void resolveRound(ResolvableUnitigGraph& resolvableGraph, std::vector<PathGroup>& readPaths, const HashList& hashlist, const size_t minCoverage, const size_t maxResolveLength, const size_t maxUnconditionalResolveLength, const bool guesswork, const bool copycountFilterHeuristic, const bool onlyLocalResolve, const bool doCleaning, std::ostream& log)
{
	...
	while (queue.size() > 0)
	{
		...
		checkValidity(resolvableGraph, readPaths);
		log << "try resolve k=" << topSize;
		auto resolutionResult = resolve(resolvableGraph, hashlist, readPaths, resolvables, minCoverage, topSize < maxUnconditionalResolveLength, guesswork, copycountFilterHeuristic, log);
		log << ", replaced " << resolutionResult.nodesResolved << " nodes with " << resolutionResult.nodesAdded << " nodes";
		...
		if (unitigified > 0)
		{
			log << ", unitigified " << unitigified << " nodes to " << unitigifiedTo << " nodes";
		}
		log << std::endl;
		if (resolutionResult.maybeTrimmable.size() > 0)
		{
			...
			std::unordered_set<size_t> maybeUnitigifiable = trimNodes(maybeTrimmable, resolvableGraph, readPaths, log);
			if (maybeUnitigifiable.size() > 0)
			{
				std::vector<size_t> deterministicUnitigifiable { maybeUnitigifiable.begin(), maybeUnitigifiable.end() };
				std::sort(deterministicUnitigifiable.begin(), deterministicUnitigifiable.end());
				auto unitigifiedHere = unitigifySet(resolvableGraph, readPaths, deterministicUnitigifiable);
				for (size_t i = 0; i < unitigifiedHere.size(); i++)
				{
					assert(unitigifiedHere[i].first.size() > 0);
					if (unitigifiedHere[i].first.size() > 1)
		...
		if (guesswork && doCleaning)
		{
			...
			{
				log << "removed " << removed.nodesRemoved << " nodes and " << removed.edgesRemoved << " edges" << std::endl;
		...
		if (doCleaning)
		{
			...
			{
				log << "removed " << removed.nodesRemoved + removed2.nodesRemoved << " tips and " << removed.edgesRemoved + removed2.edgesRemoved + removed3.edgesRemoved + removed4.edgesRemoved << " edges" << std::endl;
		...
		if (nodesRemoved > resolvableGraph.unitigs.size() / 2 + 50)
		{
			...
			log << "compacted from " << oldSize << " to " << newSize << std::endl;
			checkValidity(resolvableGraph, readPaths);
...
{
	auto resolvableGraph = getUnitigs(initial, minCoverage, hashlist, kmerSize, keepGaps, rawReadPaths, log);
	...
	std::vector<PathGroup> readPaths;
	{
		...
		for (size_t i = 0; i < rawReadPaths.size(); i++)
		{
			...
			readPaths.back().reads.back().forced = rawReadPaths[i].forced;
	...
	log << rawReadPaths.size() << " raw read paths" << std::endl;
	log << readPaths.size() << " read paths" << std::endl;
	for (size_t i = 0; i < resolvableGraph.unitigs.size(); i++)
	{
	...
	resolveRound(resolvableGraph, readPaths, hashlist, minCoverage, maxResolveLength, maxUnconditionalResolveLength, guesswork, copycountFilterHeuristic, onlyLocalResolve, doCleaning, log);
	resolveRound(resolvableGraph, readPaths, hashlist, 1, maxResolveLength, maxUnconditionalResolveLength, guesswork, copycountFilterHeuristic, onlyLocalResolve, doCleaning, log);
	checkValidity(resolvableGraph, readPaths);
