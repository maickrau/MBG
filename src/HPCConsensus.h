...
void getHpcVariants(const HashList& hash, const UnitigGraph& unitigs, const std::vector<ReadPath>& readPaths, const size_t kmerSize, ReadpartIterator& partIterator, const size_t numThreads, const double minUnitigCoverage, const double maxUnitigCoverage, const size_t minVariantCoverage);
