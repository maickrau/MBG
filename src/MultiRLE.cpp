...
std::pair<uint16_t, uint16_t> getCodeAndRunlength(const std::string& str, size_t start, size_t end, uint16_t motifLength)
{
	...
	uint16_t runLength = ((end - start) - overhang) / motifLength;
...
std::pair<std::vector<uint16_t>, std::vector<uint16_t>> multiRLECompressOne(const std::string& str)
{
	...
	for (size_t i = 0; i < str.size(); i++)
	{
		...
		for (size_t motifLength = 2; motifLength <= MaxMotifLength; motifLength++)
		{
			...
			size_t runLength = 1;
			while (i+motifLength*runLength+motifLength <= str.size() && str.substr(i, motifLength) == str.substr(i+motifLength*runLength, motifLength))
			{
				runLength += 1;
			}
			assert(runLength >= 2);
	...
	std::pair<std::vector<uint16_t>, std::vector<uint16_t>> result;
	...
	for (size_t i = 0; i < str.size(); i++)
	{
		switch(str[i])
		{
			...
			case 'T':
				break;
			default:
				if (i > lastBreak+1) result.emplace_back(multiRLECompressOne(str.substr(lastBreak, i)));
				lastBreak = i;
				break;
		}
	}
	if (lastBreak != str.size()-1) result.emplace_back(multiRLECompressOne(str.substr(lastBreak)));
	return result;
}
...
std::vector<uint16_t> revCompMultiRLE(const std::vector<uint16_t>& codes)
{
	...
	for (size_t i = 0; i < result.size(); i++)
	{
		result[i] = reverseComplement(codes[codes.size()-1-i]);
	}
	return result;
}
