...
void collectEndSmers(std::vector<bool>& endSmer, const std::vector<std::string>& files, const size_t kmerSize, const size_t windowSize, const bool hpc, const bool dinucRle)
{
	...
	for (const std::string& filename : files)
	{
		...
		FastQ::streamFastqFromFile(filename, false, [&endSmer, smerSize, hpc, dinucRle, &addedEndSmers](FastQ& read)
		{
...
void loadReadsAsHashesMultithread(HashList& result, const std::vector<std::string>& files, const size_t kmerSize, const size_t windowSize, const bool hpc, const bool collapseRunLengths, const size_t numThreads, const bool dinucRle, const bool includeEndKmers)
{
	...
	if (includeEndKmers)
	{
		...
		collectEndSmers(endSmer, files, kmerSize, windowSize, hpc, dinucRle);
	}
	...
	{
		threads.emplace_back([&readDone, &endSmer, &result, &sequenceQueue, &totalNodes, hpc, dinucRle, kmerSize, windowSize, collapseRunLengths, includeEndKmers]()
		{
			...
			while (true)
			{
				...
				for (size_t i = 0; i < parts.size(); i++)
				{
					...
					{
						minHash = findSyncmerPositions(seq, kmerSize, kmerSize - windowSize + 1, smerOrder, [](uint64_t hash) { return false; }, [&positions, &seq, kmerSize, hpc](size_t pos)
						{
...
void writePaths(const HashList& hashlist, const UnitigGraph& unitigs, const std::vector<std::string>& inputReads, const size_t kmerSize, const size_t windowSize, const bool hpc, const bool dinucRle, const bool includeEndKmers, const std::string& outputSequencePaths, const size_t numThreads)
{
	...
	for (size_t i = 0; i < numThreads; i++)
	{
		threads.emplace_back([&readDone, &outPaths, kmerSize, hpc, dinucRle, &approxHashes, &exactHashes, &kmerUnitigPosition, &unitigLength, &kmerUnitigStart, &kmerUnitigEnd, &hashlist, &unitigs, &sequenceQueue, &pathWriteMutex](){
			while (true)
			{
				std::shared_ptr<FastQ> read;
				if (!sequenceQueue.try_dequeue(read))
...
void runMBG(const std::vector<std::string>& inputReads, const std::string& outputGraph, const size_t kmerSize, const size_t windowSize, const size_t minCoverage, const double minUnitigCoverage, const bool hpc, const bool collapseRunLengths, const bool blunt, const size_t numThreads, const bool dinucRle, const bool includeEndKmers, const std::string& outputSequencePaths)
{
	auto beforeReading = getTime();
	HashList reads { kmerSize, collapseRunLengths, numThreads };
	loadReadsAsHashesMultithread(reads, inputReads, kmerSize, windowSize, hpc, collapseRunLengths, numThreads, dinucRle, includeEndKmers);
	...
	if (outputSequencePaths != "")
	{
		...
		writePaths(reads, unitigs, inputReads, kmerSize, windowSize, hpc, dinucRle, includeEndKmers, outputSequencePaths, numThreads);
	}
